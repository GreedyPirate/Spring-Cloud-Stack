    ...
    public static void type() {
        ...
        for (int i = 0; i < methods.length; i++) {
            ...
            if (method.getName().equals("test")) {
                ...
                Class<TestReflect> clazz = (Class<TestReflect>) params[0];
                System.out.println("clazz = " + clazz.getName());

                ParameterizedType parameterizedType = (ParameterizedType) params[1];
                String typeName = parameterizedType.getTypeName();
                System.out.println("param_1 typeName = " + typeName);
                Type rawType = parameterizedType.getRawType();
                Class<List> param1_raw = (Class<List>) rawType;
                System.out.println("param1_raw = " + param1_raw.getName());
                Type ownerType = parameterizedType.getOwnerType();
                Type[] actualTypeArguments = parameterizedType.getActualTypeArguments();
                Type param_1_actual_1 = actualTypeArguments[0];
                System.out.println("param_1_actual_1 = " + param_1_actual_1);
            }
        }
    }
